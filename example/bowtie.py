...
type=bool,help="""query input files are FASTQ .fq/.fastq (default)
""",)
arg_f = parser.add_argument("-f",
type=bool,help="""query input files are (multi-)FASTA .fa/.mfa
""",)
arg_r = parser.add_argument("-r",
type=bool,help="""query input files are raw one-sequence-per-line
""",)
arg_c = parser.add_argument("-c",
type=bool,help="""query sequences given on cmd line (as <mates>, <singles>)
""",)
arg_C = parser.add_argument("-C",
type=bool,help="""reads and index are in colorspace
""",)
arg_Q = parser.add_argument("-Q",
...
type=bool,help="""--Q2 <file>   same as -Q, but for mate files 1 and 2 respectively
""",)
...
type=bool,help="""input quals are Phred+33 (default)
""",)
arg_phred64_quals = parser.add_argument("--phred64-quals",
type=bool,help="""input quals are Phred+64 (same as --solexa1.3-quals)
""",)
arg_solexa_quals = parser.add_argument("--solexa-quals",
type=bool,help="""input quals are from GA Pipeline ver. < 1.3
""",)
arg_solexa1_3_quals = parser.add_argument("--solexa1.3-quals",
type=bool,help="""input quals are from GA Pipeline ver. >= 1.3
""",)
arg_integer_quals = parser.add_argument("--integer-quals",
type=bool,help="""qualities are given as space-separated integers (not ASCII)
""",)
arg_large_index = parser.add_argument("--large-index",
type=bool,help="""force usage of a 'large' index, even if a small one is present
Alignment:
""",)
...
type=bool,help="""disable Maq-like quality rounding for -n (nearest 10 <= 30)
""",)
...
type=bool,help="""--rf/--ff     -1, -2 mates align fw/rev, rev/fw, fw/fw (default: --fr)
""",)
arg_nofw = parser.add_argument("--nofw",
type=bool,help="""--norc      do not align to forward/reverse-complement reference strand
""",)
...
type=bool,help="""--tryhard       try hard to find valid alignments, at the expense of speed
""",)
...
type=bool,help="""--all           report all alignments per read (much slower than low -k)
""",)
...
type=bool,help="""hits guaranteed best stratum; ties broken by quality
""",)
arg_strata = parser.add_argument("--strata",
type=bool,help="""hits in sub-optimal strata aren't reported (requires --best)
Output:
""",)
arg_t = parser.add_argument("-t",
type=bool,help="""--time          print wall-clock time taken by search phases
""",)
...
type=bool,help="""print nothing but the alignments
""",)
arg_refout = parser.add_argument("--refout",
type=bool,help="""write alignments to files refXXXXX.map, 1 map per reference
""",)
arg_refidx = parser.add_argument("--refidx",
type=bool,help="""refer to ref. seqs by 0-based index rather than name
""",)
arg_al = parser.add_argument("--al",
type=bool,help="""<fname>       write aligned reads/pairs to file(s) <fname>
""",)
arg_un = parser.add_argument("--un",
type=bool,help="""<fname>       write unaligned reads/pairs to file(s) <fname>
""",)
arg_max = parser.add_argument("--max",
type=bool,help="""<fname>      write reads/pairs over -m limit to file(s) <fname>
""",)
arg_suppress = parser.add_argument("--suppress",
type=bool,help="""<cols>  suppresses given columns (comma-delim'ed) in default output
""",)
arg_fullref = parser.add_argument("--fullref",
type=bool,help="""write entire ref name (default: only up to 1st space)
Colorspace:
""",)
...
type=bool,help="""<dec>    approx. fraction of SNP bases (e.g. 0.001); sets --snpphred
""",)
arg_col_cseq = parser.add_argument("--col-cseq",
type=bool,help="""print aligned colorspace seqs as colors, not decoded bases
""",)
arg_col_cqual = parser.add_argument("--col-cqual",
type=bool,help="""print original colorspace quals, not decoded quals
""",)
arg_col_keepends = parser.add_argument("--col-keepends",
type=bool,help="""keep nucleotides at extreme ends of decoded alignment
SAM:
""",)
arg_sam = parser.add_argument("-S",
type=bool,help="""--sam           write hits in SAM format
""",)
...
type=bool,help="""supppress header lines (starting with @) for SAM output
""",)
arg_sam_nosq = parser.add_argument("--sam-nosq",
type=bool,help="""supppress @SQ header lines for SAM output
""",)
...
type=bool,help="""use memory-mapped I/O for index; many 'bowtie's can share
""",)
arg_shmem = parser.add_argument("--shmem",
type=bool,help="""use shared mem for index; many 'bowtie's can share
Other:
""",)
...
type=bool,help="""verbose output (for debugging)
""",)
