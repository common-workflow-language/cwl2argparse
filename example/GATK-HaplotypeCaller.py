...
arg_outputfile_HaplotypeCaller = parser.add_argument("-o",
...
type=bool,help="""Use the contamination-filtered read maps for the purposes of annotating variants""",)
arg_useAllelesTrigger = parser.add_argument("--useAllelesTrigger",
type=bool,help="""Use additional trigger on variants found in an external alleles f""",)
...
type=bool,help="""If provided, all bases will be tagged as active""",)
...
type=bool,help="""If specified, we will not trim down the active region from the full region (active + extension) to just the active interval for genotyping""",)
arg_dontIncreaseKmerSizesForCycles = parser.add_argument("--dontIncreaseKmerSizesForCycles",
type=bool,help="""Disable iterating over kmer sizes when graph cycles are detected""",)
arg_doNotRunPhysicalPhasing = parser.add_argument("--doNotRunPhysicalPhasing",
type=bool,help="""As of GATK 3.3, HaplotypeCaller outputs physical (read-based) information (see version 3.3 release notes and documentation for details). This argument disables that behavior.""",)
arg_disableOptimizations = parser.add_argument("--disableOptimizations",
type=bool,help="""Dont skip calculations in ActiveRegions with no variants""",)
arg_debug = parser.add_argument("--debug",
type=bool,help="""Print out very verbose debug information about each triggering active region""",)
...
arg_bandPassSigma = parser.add_argument("--consensus",
...
type=bool,help="""If provided, we will annotate records with the number of alternate alleles that were discovered (but not necessarily genotyped) at a given site""",)
arg_allSitePLs = parser.add_argument("--allSitePLs",
type=bool,help="""Annotate all sites with PLs""",)
arg_allowNonUniqueKmersInRef = parser.add_argument("--allowNonUniqueKmersInRef",
type=bool,help="""Allow graphs that have non-unique kmers in the reference""",)
